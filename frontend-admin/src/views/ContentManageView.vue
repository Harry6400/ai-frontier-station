...
const AI_SOURCE_TEMPLATES = {
  ...
  manual: {
    ...
    instruction: '请按内容平台导读方式总结，保持中性、可信、适合正式发布。'
  }
}
...
function resetGitHubImportForm() {
  ...
  githubImportForm.publishStatus = 'PUBLISHED'
...
async function generateAiSourceSummary() {
  ...
  try {
    ...
    ElMessage.success('AI 导读生成成功，请确认内容后继续')
  } catch (error) {
...
async function submitAiSourceImport() {
  await aiSourceFormRef.value.validate()
  if (!aiSourceGenerated.value) {
    ElMessage.warning('请先生成 AI 导读')
    return
  }
  try {
    await importAiSource(buildAiSourceImportPayload())
    ElMessage.success('AI 来源整理内容已发布')
...
function resetArxivImportForm() {
  ...
  arxivImportForm.publishStatus = 'PUBLISHED'
...
function resetHuggingFaceImportForm() {
  ...
  huggingfaceImportForm.publishStatus = 'PUBLISHED'
  ...
  <section class="admin-page" v-loading="loading">
          ...
          >
            ...
            <el-form-item label="整理要求">
              <el-input
                ...
                type="textarea"
                :rows="2"
                placeholder="例如：偏工程实践、偏科研动态、偏产品分析"
              />
            </el-form-item>
          </el-form>
        </section>
    ...
    >
      <div class="editor-guide github-import-guide">
        ...
        <p>
          通过 arXiv API 搜索 AI 相关论文，选择候选后回填表单，再导入为内容和外部引用记录。
        </p>
      </div>
